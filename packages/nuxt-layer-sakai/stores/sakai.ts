...
export function useSakaiStore(options?: SakaiOptions) {
   ...
   * 静态数据
   */
  const presets: Record<SakaiThemePresetName, any> = {
    Aura,
    Lara,
    Nora,
  }

  const primaryColorList: SakaiPrimaryColorItem[] = [
    { name: 'noir', palette: {} },
  ...
  const surfaceList: SakaiSurfaceItem[] = [
  ...
  watch(() => config.value.preset, (_preset) => {
    const presetValue = presets[_preset]
    const surfacePalette = surfaceList.find(s => s.name === config.value.surface)?.palette
    $t()
      .preset(presetValue)
      .preset(getPresetExt())
  ...
  watchImmediate(() => config.value.surface, () => {
    const surface = surfaceList.find(s => s.name === config.value.surface)
  ...
  function getPresetExt() {
    const color = primaryColorList.find(c => c.name === config.value.primary) ?? primaryColorList[0]!
