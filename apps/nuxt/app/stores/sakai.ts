...
export const useSakaiStore = defineStore('sakai', () => {
   ...
   * 静态数据
   */
  const presets: Record<SakaiConfig['preset'], any> = {
    Aura,
    Lara,
  }

  const primaryColors: SakaiPrimaryColorItem[] = [
    { name: 'noir', palette: {} },
  ...
  const surfaces: SakaiSurfaceItem[] = [
    {
      name: 'slate',
  ...
  watch(() => config.value.preset, (_preset) => {
    ...
    const surfacePalette = surfaces.find(s => s.name === config.value.surface)?.palette
  ...
  watchImmediate(() => config.value.surface, () => {
    const surface = surfaces.find(s => s.name === config.value.surface)
  ...
  function getPresetExt() {
    const color = primaryColors.find(c => c.name === config.value.primary) ?? primaryColors[0]!
