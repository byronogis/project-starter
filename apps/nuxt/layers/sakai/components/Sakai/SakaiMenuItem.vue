...
<template>
  <li data-allow-mismatch :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
    <template v-if="!item.hidden">
      ...
      <a v-if="(!item.to || item.items)" :href="item.url" :class="item.class" :target="item.target" tabindex="0" @click="itemClick($event, item, index)">
        <i :class="item.icon" class="layout-menuitem-icon" />
        <span class="layout-menuitem-text">{{ item.label }}</span>
        <i v-if="item.items" class="layout-submenu-toggler i-prime:angle-down" />
      </a>

      <router-link v-if="item.to && !item.items" :class="[item.class, { 'active-route': checkActiveRoute(item) }]" tabindex="0" :to="item.to" @click="itemClick($event, item, index)">
        <i :class="item.icon" class="layout-menuitem-icon" />
        <span class="layout-menuitem-text">{{ item.label }}</span>
      ...
      <Transition v-if="item.items" name="layout-submenu">
        <ul v-show="root ? true : isActiveMenu" class="layout-submenu">
          <SakaiMenuItem v-for="(child, i) in item.items" :key="child.to" :index="i" :item="child" :parent-item-key="itemKey" :root="false" />
        </ul>
      </Transition>
    </template>
  </li>
